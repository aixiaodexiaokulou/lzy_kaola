...
$(function () {
    ...
    //改写添加ajax
    function changeIptBorder1(regResult) {
        var that = $(this)
        if (regResult) {
            $.get('/checkaccount/', {'account': $(this).val()}, function (response) {
                ...
                if (response.status == 1) {
                    $('.user-count i').hide()
                    $(that).removeClass('errorBorder').css('border', '1px solid rgb(15, 157, 85)')
                } else {
                    $(that).addClass('errorBorder');
                    $('.user-count i').show()
                    $('.user-count i').html(response.msg).css('color', 'red')
                }
            })
        } else {
    ...
    $('#user-count').blur(function () {
        ...
        changeIptBorder1.call(this, regCount.test(countVal));
    })
    ...
    //改写添加ajax(手机号验证)
    function changeIptBorder2(regResult) {
        var that = $(this)
        if (regResult) {
            $.get('/checktel/', {'tel': $(this).val()}, function (response) {
                ...
                if (response.status == 1) {
                    ...
                    $(that).addClass('errorBorder');
                    $('.user-phone i').show()
                    $('.user-phone i').html(response.msg).css('color', 'red')
                }
            })
        } else {
            $(this).addClass('errorBorder');
    ...
    $('#user-phone').blur(function () {
        ...
        changeIptBorder2.call(this, regPhone.test(phoneVal));
    })
